...
// Using any here to avoid tight coupling with API response variations
...
export function ExamPage() {
  ...
  const [examSession, setExamSession] = useState<any | null>(null);
  ...
  const startExam = async () => {
    ...
    setLoading(true);
    try {
      const session = await examApi.startExam(subjectId);
      setExamSession(session);
      setTimeRemaining(session.timeRemaining);
      
      // Set initial answers object with empty values for all questions
      const initialAnswers = session.questions.reduce((acc, q) => ({
        ...acc,
        [q.id]: -1 // -1 means no answer selected
      }), {});
